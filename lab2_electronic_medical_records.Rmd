...
note_id: ID that is assigned to a piece of data
description: about a patient or related terms to them
medical_specailty: Related words to symptoms of a patient
sample_name: Treatments
transcription: Note for the patient. Subjective, past medical history, preoperative diagnosis, etc
keywords: the keywords related to the patient
...
```{r}
tokenized.data.unique <- analysis.data %>% tidytext::unnest_tokens(word, transcription, token = "sentences", to_lower=TRUE)

tokenized.data.unique %>% dplyr::group_by(medical_specialty) %>% dplyr::distinct(word) %>% dplyr::summarise(n=dplyr::n())
```
...
It could be difficult for a general purpose lemmatizer to be used for medical deta because there are many technical terms in this field and it is needed for the lemmatizer to have enough knowledge about the terms. If it only has poor knowledge, the program will not work well. 
...
"series images" is the lemma that is frequently used in both radiology and Surgery, and "carpal ligament" is the lemma that is frequently used in both Surgery and Orthopedic. Yes, this plot is reasonable because those words are compatible to each fields.
...
ggplot2::ggplot(lemma.freq, ggplot2::aes(x = proportion, 
  ...
  ggplot2::geom_text(ggplot2::aes(label = lemma), check_overlap = TRUE, vjust = 1.5) +
  ggplot2::scale_x_log10(labels = scales::percent_format()) + 
  ggplot2::scale_y_discrete(limits = c("Surgery", "Radiology")) +
  ggplot2::scale_color_gradient(limits = c(0, 0.001), low = "darkslategray4", high = "gray75") +
  ggplot2::facet_grid(medical_specialty ~ ., scales = "free_x", space = "free_x") +
  ggplot2::theme(legend.position = "none") +
...
"myocardial perfusion" has the highest tf_idf value in this lists, that means the lemma is comparatively important word in Radiology field.
...
```{r}
analysis.data %>% dplyr::select(medical_specialty, transcription) %>% dplyr::filter(!stringr::str_detect(transcription, 'Orthopedic')) %>% dplyr::filter(!stringr::str_detect(transcription, 'b.i.d')) %>% dplyr::slice(1)
```
